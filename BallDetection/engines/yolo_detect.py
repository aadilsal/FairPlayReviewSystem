...
class YOLOBallDetector:
    ...
    def detect_with_conf_override(self, img, conf):
        """Rescue pass: runs Model 1 with an arbitrary low-confidence threshold."""
        iou = DETECTION_CONFIG['iou_threshold']
        imgsz = GLOBAL_CONFIG['imgsz']
        return self.detect(img, conf=conf, iou=iou, imgsz=imgsz)
...
def yolo_detect_ball_lowconf(detector, frame, conf):
    """Wrapper function for rescue pass detection with overridable confidence."""
    return detector.detect_with_conf_override(frame, conf)